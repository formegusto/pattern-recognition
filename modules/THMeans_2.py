...
class THMeans:
    ...
    def get_divide_index(self, K, length):
        ...
        pattern_idxes = []
        pattern_idxes.append(self.mean_pattern.values)
        pattern_idxes.append(self.datas.iloc[:, idxes[1]].values)

        # print("pattern_idxes:", pattern_idxes)
        # K
        d_weight = 1
        d_weight_sum = 1
        while True:
            ...
            for dseq in range(0, d_weight):
                ...
                if len(idxes) != (dseq + 1):
                    ...
                    # pattern_idxes.append(
                    #     self.datas.iloc[:, [tmp[dseq], tmp[dseq + 1]]].T.mean().values)
            ...
            if len(idxes) >= K:
                ...
                d_weight_sum += 1
        idxes.remove(0)
        # idxes.remove(arr[length - 1])
        idxes.sort()
        idxes.reverse()
        # pattern_idxes = pattern_idxes[0: K]
        return idxes
    ...
    def init_cluster(self):
        ...
        self.cluster_dict[0] = self.mean_pattern.values

        # print("-{}: K Setting Okay".format(2))
        # self.cluster_dict[1] = self.cluster_dict[len(
        #     self.cluster_dict.keys()
        # )] = self.datas.iloc[:, self.K - 1].values
        ...
        print("init_cluster", idxes)

        for idx, value in enumerate(idxes):
            self.cluster_dict[len(
                self.cluster_dict.keys()
            )] = self.datas.iloc[:, value].values
